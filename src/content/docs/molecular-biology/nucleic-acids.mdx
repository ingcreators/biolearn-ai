---
title: 核酸とは何か
description: DNAとRNAを含む核酸の基本構造、役割、ヌクレオチドとの関係を説明します。
part: molecular-biology
module: molecular-foundations
level: foundation
priority: P1
order: 5
learningPaths:
  - foundations
relatedTerms:
  - nucleic-acids
  - dna
  - rna
prerequisites:
  - /fundamentals/molecule/
---

import LearningGoals from '../../../components/LearningGoals.astro';
import Quiz from '../../../components/Quiz.astro';
import NextSteps from '../../../components/NextSteps.astro';
import LessonFigure from '../../../components/LessonFigure.astro';
import GlossaryTable from '../../../components/GlossaryTable.astro';

<LearningGoals
  items={[
    '核酸がDNAとRNAを含む分子の種類であることを理解する',
    '核酸がヌクレオチドの連なりでできていることを説明できる',
    'DNAとRNAの共通点と違いを大まかに整理できる',
  ]}
/>

核酸は、DNAやRNAのように、遺伝情報の保存や読み出しに関わる分子の総称です。

核酸はヌクレオチドが鎖のようにつながった高分子で、塩基の並びとして情報を持ちます。代表的な核酸には、遺伝情報を長期的に保存するDNAと、情報の利用や調節に関わるRNAがあります。

<LessonFigure
  src="/figures/nucleic-acids-concept.jpg"
  alt="DNAとRNAがヌクレオチドの連なりとして表され、遺伝情報の保存と利用につながる様子を示す教材イラスト"
  title="核酸は情報を持つ長い分子である"
  caption="DNAとRNAはどちらも核酸で、ヌクレオチドが連なった分子として細胞内の情報を支えます。"
/>

## なぜ核酸の視点が重要か

核酸の視点が重要なのは、[DNA](/glossary/dna/)と[RNA](/glossary/rna/)を別々の暗記事項ではなく、情報を持つ分子としてまとめて理解できるからです。どちらもヌクレオチドからできていると分かると、転写、PCR、RNA-seq、[シーケンシング](/glossary/sequencing/)の前提が見えやすくなります。

核酸は実験でも頻繁に扱われます。DNAを増幅するPCR、RNA量を測るRT-qPCR、配列を読むシーケンシングなどは、すべて核酸を対象にした方法です。

## どんな核酸があるか

DNAは主に安定な情報保存に向いている核酸です。真核細胞では、核の中の染色体に多くのDNAが含まれています。

RNAは、情報を使う場面で柔軟に働く核酸です。mRNA、rRNA、tRNA、非コードRNAなど複数の種類があり、タンパク質合成や遺伝子発現の調節に関わります。

## 核酸はどう調べるか

実験では、サンプルからDNAやRNAを抽出し、目的に応じて[PCR](/glossary/pcr/)、RT-qPCR、RNA-seq、DNAシーケンシングなどを行います。このとき「核酸濃度」や「核酸品質」という表現が出てくることがあります。

どの方法を使うかは、DNA配列を読みたいのか、RNA量を測りたいのか、特定の領域を増やしたいのかで変わります。核酸を扱う実験では、DNA由来の情報とRNA由来の情報を区別して読むことが大切です。

## 核酸の変化は何につながるか

DNA配列の変化は、遺伝子の働きやタンパク質のアミノ酸配列に影響する場合があります。ただし、配列の変化が見つかったことと、細胞や個体の性質の原因だといえることは同じではありません。

RNA量やRNAの種類の変化は、遺伝子発現の状態を読む手がかりになります。RNA-seqなどで見える差は、細胞の状態、サンプルの条件、解析方法を合わせて解釈します。

## 論文や実験ではどう出てくるか

核酸は[Methods](/glossary/methods/)で「DNA/RNA extraction」「nucleic acid purification」「library preparation」のように出てきます。[Results](/glossary/results/)では、DNA配列、RNA発現量、変異、リード数などとして表されます。

Figureを読むときは、そのデータがDNAを読んだものなのか、RNA量を測ったものなのかを確認します。同じ「配列データ」でも、DNA由来とRNA由来では解釈が変わります。

## どんな点でつまずきやすいか

### 似た用語との区別

- 核酸とヌクレオチド: 核酸はヌクレオチドが多数つながった分子で、ヌクレオチドはその構成単位です。
- 核酸とDNA: DNAは核酸の一種です。核酸にはRNAも含まれます。
- DNAとRNA: どちらも核酸ですが、使う糖や塩基、細胞内での役割が異なります。

### 解釈の落とし穴

- 核酸はDNAだけを指す言葉ではありません。RNAも核酸です。
- RNAはDNAの単なるコピーではありません。RNAとして機能する分子も多くあります。
- 核酸を測った結果が、すぐに細胞の機能を完全に説明するわけではありません。

## 重要用語

<GlossaryTable
  terms={[
    {
      term: '核酸',
      english: 'nucleic acid',
      description: 'DNAやRNAのように、ヌクレオチドが連なってできた情報分子。',
      href: '/glossary/nucleic-acids/',
    },
    {
      term: 'ヌクレオチド',
      english: 'nucleotide',
      description: '塩基、糖、リン酸からなる、DNAやRNAなど核酸の構成単位。',
      href: '/glossary/nucleotide/',
    },
    {
      term: 'DNA',
      english: 'deoxyribonucleic acid',
      abbreviation: 'DNA',
      description: '遺伝情報を長期的に保存する分子。',
      href: '/glossary/dna/',
    },
    {
      term: 'RNA',
      english: 'ribonucleic acid',
      abbreviation: 'RNA',
      description: '遺伝情報の読み出しや調節に関わる分子。',
      href: '/glossary/rna/',
    },
    {
      term: 'シーケンシング',
      english: 'sequencing',
      description: 'DNAやRNA由来の塩基配列を読み取る技術の総称。',
      href: '/glossary/sequencing/',
    },
  ]}
/>

<Quiz
  questions={[
    {
      question: '核酸に含まれる代表的な分子を2つ挙げてください。',
      answer: 'DNAとRNAです。',
    },
    {
      question: '核酸を構成する基本単位は何ですか。',
      answer: 'ヌクレオチドです。',
    },
    {
      question: 'DNAとRNAの役割の違いを簡単に説明してください。',
      answer:
        'DNAは主に遺伝情報の保存に関わり、RNAは情報の読み出しや調節、タンパク質合成に関わります。',
    },
    {
      question: '核酸を扱う実験法を一つ挙げてください。',
      answer: 'PCR、RT-qPCR、RNA-seq、シーケンシングなどがあります。',
    },
    {
      question: '核酸とヌクレオチドの関係を説明してください。',
      answer: '核酸はヌクレオチドが多数つながってできた分子です。',
    },
  ]}
/>

<NextSteps
  items={[
    { title: 'ヌクレオチドとは何か', href: '/molecular-biology/nucleotide/' },
    { title: 'DNAとは何か', href: '/fundamentals/dna/' },
    { title: 'RNAとは何か', href: '/fundamentals/rna/' },
  ]}
/>
