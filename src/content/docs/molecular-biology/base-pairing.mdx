...
塩基対の視点が重要なのは、DNAの二本鎖構造、DNA複製、転写、PCR、シーケンシングを同じ「相補性」の考え方でつなげられるからです。片方の鎖の配列が分かると、もう片方の鎖で対応する塩基を推定できます。
...
塩基対が正しく対応しないミスマッチは、複製エラーやDNA損傷の修復を考えるときに出てきます。細胞にはミスマッチや損傷を検出して直す仕組みがありますが、すべてを単純なA-T、G-Cの対応だけで説明できるわけではありません。
