---
title: Figure legendの読み方
description: 論文Figureの説明文から、実験条件、n数、反復、統計検定、エラーバーやp値の定義を読み取る基本を説明します。
part: figures
module: figure-basics
level: foundation
priority: P0
order: 10
learningPaths:
  - figure-reading
relatedTerms:
  - figure-legend
  - variable
  - sample-size
  - biological-technical-replicates
prerequisites: []
---

import LearningGoals from '../../../components/LearningGoals.astro';
import Quiz from '../../../components/Quiz.astro';
import NextSteps from '../../../components/NextSteps.astro';
import LessonFigure from '../../../components/LessonFigure.astro';

<LearningGoals
  items={[
    'Figure legendで実験条件、n数、統計表示を探せる',
    '図だけでは分からない前提を本文やMethodsに戻して確認できる',
    'エラーバーやp値の定義を読み間違えにくくする',
  ]}
/>

Figure legend（図の説明文）は、Figureの横や下に置かれる短い説明です。図だけでは分からない実験条件、サンプル数、統計検定、[エラーバー](/glossary/error-bar/)の意味などが書かれています。

Figureを見るときは、legendを図の読み方を案内する地図として使います。図を見る前に前提をつかみ、見たあとに条件や統計表示を確かめ、迷ったときは本文やMethodsへ戻ります。

<LessonFigure
  src="/figures/figure-legend-concept.jpg"
  alt="Figure legendから条件、n数、統計表示、Methodsへの手がかりを読み取る概念図"
  title="Figure legendは図を読むための地図"
  caption="図の見た目だけで判断せず、条件、反復、統計、補足情報をlegendから拾います。"
/>

## この文章で何を見るか

この文章では、図に示されたデータが「何を、どの条件で、どのように測った結果なのか」を確認します。パネル、測定対象、条件、n数、統計表示、補足情報の場所を拾うのが主な目的です。

たとえば、[棒グラフ](/glossary/bar-plot/)だけを見ると条件Aと条件Bの差は分かっても、細胞種、処理時間、濃度、反復数、エラーバーの定義は分からないことがあります。Figure legendを読むことで、図の見た目を実験の文脈に戻して理解できます。

論文によっては、Figure legendに詳しい方法が書かれ、細かい実験手順はMethodsに分けて書かれます。Figure legendだけで分からないときは、本文とMethodsも合わせて確認します。

## まず確認する項目

最初に、次の項目を探します。

- 図のパネル: Figure 1A、Figure 1Bなど、どの説明がどのパネルに対応するか。
- 測定対象: 遺伝子発現、タンパク質量、細胞数、蛍光強度など、何を測ったか。
- 比較している条件: control、treated、knockdown、time pointなど、何と何を比べているか。
- サンプルの種類: 細胞株、組織、個体、患者由来サンプル、公開データなど。
- 表示方法: 平均、中央値、fold change、z-score、正規化値など。
- 補足の場所: Methods、補足Figure、Source Dataに追加情報がないか。

特に複数パネルのFigureでは、A、B、Cの説明を取り違えないことが大切です。

## 読む順番

1. Figure全体のタイトルや最初の文から、何を示す図かを確認します。
2. パネル名を追い、A、B、Cなどの説明を図中の場所と対応づけます。
3. 測定対象、サンプル、条件、処理時間、単位を拾います。
4. n数、反復の種類、統計検定、エラーバーの定義を確認します。
5. 分からない条件や手順は、MethodsやSupplementary informationに戻ります。

## 実験条件の読み方

実験条件では、「どの条件が基準で、何を変えた実験か」を確認します。

よく出てくる条件には、control（対照）、vehicle（溶媒対照）、treated（処理あり）、mock（模擬処理）、wild type（野生型）、mutant（変異体）、knockdown（発現低下）、overexpression（過剰発現）などがあります。

読むときは、次の順番で整理すると迷いにくくなります。

1. 基準条件がどれかを確認します。
2. 変えた要因が何かを確認します。薬剤、遺伝子、時間、濃度、細胞種などです。
3. 時間や濃度の単位を確認します。
4. Figure内のラベルとlegendの説明が対応しているかを確認します。

たとえば「cells were treated with 10 ng/mL X for 24 h」とあれば、薬剤や刺激Xを10 ng/mLで24時間処理した条件だと読めます。図の差を見る前に、何を変えた結果なのかを押さえます。

## n数・反復・統計検定の読み方

n数は、データの数やサンプル数を示す重要な情報です。ただし、nが何を数えているかはFigure legendで確認する必要があります。

- biological replicates（生物学的反復）: 独立したサンプル、個体、培養、実験日に由来する反復です。
- technical replicates（技術的反復）: 同じサンプルを複数回測った反復です。
- independent experiments（独立実験）: 別々に行った実験の回数を示します。
- cells counted（カウントした細胞数）: 細胞数としてのnであり、独立サンプル数とは別の場合があります。

統計検定では、どの検定を使ったか、どの群どうしを比較したか、多重検定補正をしたかを確認します。t-test、ANOVA、Mann-Whitney test、Wilcoxon test、adjusted p-valueなどの語が手がかりになります。

nが大きく見えても、同じサンプル内の細胞を多数数えただけの場合、独立した生物学的反復が多いとは限りません。Figure legendに「n = 3 independent experiments」などとあるかを探します。

## エラーバーやp値の定義を探す

エラーバーは、ばらつきや推定の不確かさを示します。ただし、同じ形でも意味は論文によって異なります。

Figure legendでは、次の表現を探します。

- mean ± SD: 平均 ± 標準偏差です。データのばらつきを表します。
- mean ± SEM: 平均 ± 標準誤差です。平均値の推定の不確かさを表します。
- 95% CI: 95%信頼区間です。
- box plotの定義: 中央値、四分位範囲、ひげ、外れ値の扱いを確認します。
- p値やアスタリスク: `* p < 0.05`、`** p < 0.01`など、記号の意味を確認します。
- ns: not significantの略で、統計的に有意とはいえないことを示す場合があります。

アスタリスクがあるときは、どの2群の比較に対する記号なのかを確認します。棒の上に線がある場合は線で結ばれた群、Figure legendに説明がある場合はその記述に従います。

## 前提に戻る

Figure legendで読める情報には限りがあります。条件や統計表示が曖昧なときは、本文とMethodsに戻って確認します。

- 実験手順や解析条件は[Methodsの読み方](/papers/how-to-read-methods/)で確認します。
- FigureがResults本文のどの主張を支えているかは[Resultsの読み方](/papers/how-to-read-results/)で確認します。
- エラーバーやp値の表示は[エラーバーの読み方](/statistics/error-bars/)と[p値とは何か](/statistics/p-value/)に戻ります。
- 複数パネルの対応が分かりにくい場合は[複数パネルFigureの読み方](/figures/multi-panel-figure/)で整理します。

## よくある誤解

- Figure legendは飾りではありません。図の意味を決める条件、n数、統計の情報が含まれます。
- n数が大きいほど必ず信頼できる、とは限りません。nが細胞数なのか、独立サンプル数なのかで意味が変わります。
- エラーバーは常に標準偏差とは限りません。SD、SEM、信頼区間のどれかを確認します。
- p値のアスタリスクだけで、効果の大きさや生物学的な重要性は判断できません。
- Figure legendに書かれていない情報は、MethodsやSupplementary informationに分かれていることがあります。

<Quiz
  questions={[
    {
      id: 'figure-legend-q1',
      question: 'Figure legendの役割として最も適切なものはどれですか。',
      choices: [
        {
          id: 'a',
          text: '図の見た目だけでは分からない実験条件、n数、反復、統計表示などを補う説明です。',
        },
        {
          id: 'b',
          text: '図を飾る文章なので、値や条件の解釈にはほとんど関係しません。',
        },
        {
          id: 'c',
          text: 'Results本文を読まなくても、論文全体の結論を一つに決める場所です。',
        },
        {
          id: 'd',
          text: '図中のアスタリスクがあれば、効果の大きさまで自動的に分かる印です。',
        },
      ],
      answer: 'a',
      explanation:
        'Figure legendは、測定対象、条件、n数、反復、統計検定、エラーバーの定義などを確認する入口です。',
    },
    {
      id: 'figure-legend-q2',
      question:
        'Figure legendで最初に探したい情報の組み合わせとして適切なものはどれですか。',
      choices: [
        {
          id: 'a',
          text: '測定対象、比較条件、サンプルの種類、n数、反復の種類、統計表示。',
        },
        {
          id: 'b',
          text: '図の色、文字の大きさ、パネルの並びの美しさだけ。',
        },
        {
          id: 'c',
          text: 'アスタリスクの数だけ。条件やn数は確認しなくてよい。',
        },
        {
          id: 'd',
          text: '論文タイトルだけ。Figure legendは本文を読む前には使わない。',
        },
      ],
      answer: 'a',
      explanation:
        '図の見た目を実験の文脈に戻すため、何をどの条件で測ったかと統計表示を確認します。',
    },
    {
      id: 'figure-legend-q3',
      question:
        '「n = 100 cells」と書かれているFigureで、独立した反復数も確認すべき理由はどれですか。',
      choices: [
        {
          id: 'a',
          text: '100個の細胞が同じサンプルや同じ実験に由来する可能性があり、独立サンプルが100あるとは限らないため。',
        },
        {
          id: 'b',
          text: '細胞数が100あれば、実験日は1回でも生物学的反復が十分多いと決まるため。',
        },
        {
          id: 'c',
          text: 'nが大きく見える図では、エラーバーの定義を確認する必要がなくなるため。',
        },
        {
          id: 'd',
          text: 'cellsと書かれていれば、必ず患者100人分のデータだと読めるため。',
        },
      ],
      answer: 'a',
      explanation:
        '細胞数としてのnと、独立したサンプルや実験の数は同じとは限りません。',
    },
    {
      id: 'figure-legend-q4',
      question:
        'エラーバーやアスタリスクを読むときにFigure legendで確認することはどれですか。',
      choices: [
        {
          id: 'a',
          text: 'エラーバーは常にSDなので、定義は確認しなくてよい。',
        },
        {
          id: 'b',
          text: 'アスタリスクがあるなら、どの群どうしの比較かは見なくてよい。',
        },
        {
          id: 'c',
          text: 'エラーバーがSD、SEM、95% CIのどれか、p値記号がどの比較に対応するか。',
        },
        {
          id: 'd',
          text: 'アスタリスクの数が多ければ、必ず生物学的に重要だと判断できる。',
        },
      ],
      answer: 'c',
      explanation:
        '同じ見た目のエラーバーや記号でも意味は論文ごとに異なるため、legendで定義を確認します。',
    },
  ]}
/>

<NextSteps
  items={[
    { title: '軸・ラベル・単位の読み方', href: '/figures/axis-label-unit/' },
    { title: '棒グラフの読み方', href: '/figures/how-to-read-bar-plot/' },
    { title: 'qPCR plotの読み方', href: '/figures/how-to-read-qpcr-plot/' },
    {
      title: 'Western blotの読み方',
      href: '/figures/how-to-read-western-blot/',
    },
  ]}
/>
