...
Figure legend（図の説明文）は、Figureの横や下に置かれる短い説明です。図だけでは分からない実験条件、サンプル数、統計検定、[エラーバー](/glossary/error-bar/)の意味などが書かれているため、Figureを読むときの地図になります。
...
Figure legendでは、図に示されたデータが「何を、どの条件で、どのように測った結果なのか」を確認します。パネル、測定対象、条件、n数、統計表示、補足情報の場所を拾うのが主な目的です。
...
Figure legendは、Figureの見た目を実験・解析の前提へ戻すための手がかりです。図を見る前、見たあと、迷ったときの3回読むと、条件や統計表示の読み違いを減らせます。
...
- 複数パネルの対応が分かりにくい場合は[multi-panel Figureの読み方](/figures/multi-panel-figure/)で整理します。
