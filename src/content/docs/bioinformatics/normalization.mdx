...
<Quiz
  questions={[
    {
      id: 'normalization-q1',
      question: '正規化の説明として最も適切なのはどれですか。',
      choices: [
        'サンプル間や細胞間で測定量を比較しやすくするために、数値の尺度を整える処理',
        '低品質のサンプルを必ず正しいデータに直す処理',
        '生カウントを見れば不要になる、図の見た目だけを整える処理',
        '統計的な検定を行わずに、条件差を証明する処理',
      ],
      answer:
        'サンプル間や細胞間で測定量を比較しやすくするために、数値の尺度を整える処理',
      explanation:
        '正規化は、ライブラリサイズや検出効率などの違いを考慮し、比較しやすい尺度へ整える処理です。',
    },
    {
      id: 'normalization-q2',
      question:
        'RNA-seqのカウントをそのまま比較しにくい理由として適切なのはどれですか。',
      choices: [
        '総リード数、検出効率、遺伝子長、細胞品質などが観測カウントに影響するため',
        '生カウントは、どのサンプルでも常に同じ条件で得られるため',
        '正規化後の値は生物学的結論そのものになるため',
        'カウントには技術的なばらつきが含まれないため',
      ],
      answer:
        '総リード数、検出効率、遺伝子長、細胞品質などが観測カウントに影響するため',
      explanation:
        '生カウントは測定量や技術的な違いに左右されるため、そのまま公平な比較とは限りません。',
    },
    {
      id: 'normalization-q3',
      question: '生カウントと正規化後の値の違いとして適切なのはどれですか。',
      choices: [
        '生カウントは観測された数で、正規化後の値は比較しやすいように尺度を調整した値',
        '生カウントは解析済みの結論で、正規化後の値はFASTQそのものである',
        '両者は完全に同じ意味なので、図では区別しなくてよい',
        '正規化後の値なら、どの方法でも常に同じ尺度として直接比較できる',
      ],
      answer:
        '生カウントは観測された数で、正規化後の値は比較しやすいように尺度を調整した値',
      explanation:
        '生カウントと正規化後の値は役割が違うため、図やMethodsでどちらを見ているか確認します。',
    },
    {
      id: 'normalization-q4',
      question: '正規化後の値を読むときの注意として適切なのはどれですか。',
      choices: [
        '正規化方法、対数変換、フィルタ条件、品質管理、バッチ効果の扱いを確認する',
        '正規化後なら、実験設計や反復は確認しなくてよい',
        '正規化はすべての技術的な偏りを必ず消す',
        '正規化後の値が高ければ、条件差が統計的に確かだと断定できる',
      ],
      answer:
        '正規化方法、対数変換、フィルタ条件、品質管理、バッチ効果の扱いを確認する',
      explanation:
        '正規化後の値でも、処理方法や品質、実験設計を合わせて読まないと誤解しやすくなります。',
    },
  ]}
/>
