...
分野別カテゴリに置く通常記事です。基礎概念を扱う新規 Lesson は、目次だけで学習の流れが分かるように、原則として次の構成を持ちます。
...
本文内にはページタイトルと同じ `## {概念名}とは何か` 見出しを作りません。`英語表記と略語` は独立節にせず、`重要用語` の `GlossaryTable` に日本語名、英語名、必要な略語、1文説明、用語集リンクとして集約します。`NextSteps` は次に読むレッスン記事だけを指し、用語集ページへの導線は `GlossaryTable` に置きます。
